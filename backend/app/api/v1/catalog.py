...
def _parse_homelink_list_page(html: str, base_url: str, limit: int) -> List[Dict[str, Any]]:
    """
    Homelink category pages often render product tiles as plain text:
      "<title> ... KES 19,999 ... View product"
    We'll extract (link, title, price) from anchor blocks that contain KES.
    """
    anchors = re.findall(
        r'<a[^>]+href=["\']([^"\']+)["\'][^>]*>(.*?)</a>',
        html,
    ...
    for href, inner in anchors:
        ...
        # Only consider anchors that look like product tiles (have KES on the same anchor line)
        ...
        # title = remove price bits and common noise
        title = text
...
def _parse_product_page_fallback(html: str) -> Tuple[Optional[str], Optional[str], Optional[str], Optional[Decimal]]:
    """
    Generic product page fallback when JSON-LD is missing:
      - title: <h1>...</h1> or og:title
      - sku: "SKU XYZ" lines
      - price: first "KES 12,345" occurrence
      - description: meta description or first long paragraph after title (best-effort)
    """
    # title from h1
    ...
    # sku (common pattern: "SKU HLA0180")
    sku = None
    ...
    desc = og_desc
    if not desc:
        # try first paragraph with some length
...
async def _try_woocommerce_store_api(client: httpx.AsyncClient, root: str, max_items: int) -> List[Dict[str, Any]]:
    ...
    Try WooCommerce Store API:
      GET {root}/wp-json/wc/store/v1/products?per_page=..
    """
...
async def _try_shopify_product_json(client: httpx.AsyncClient, product_url: str) -> Optional[Dict[str, Any]]:
    """
    Shopify fallback:
      /products/<handle>.js returns JSON with title + variants.
    """
...
async def scrape_catalog_items(
    ...
    async with httpx.AsyncClient(
        ...
        headers={
            "User-Agent": "POSTIKA/1.0 (+https://postika.co.ke) product-scraper",
            "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
        },
    ) as client:
        # 0) Woo Store API probe (fastest)
        if payload.try_woocommerce_store_api:
            try:
                ...
                # ignore and proceed to HTML scraping
                pass

        # 1) Fetch initial URL HTML (only if nothing created yet)
        if not created:
            try:
                status_code, html = await _fetch(client, url)
            except httpx.RequestError as e:
                raise HTTPException(
                    status_code=502,
                    detail=f"Failed to fetch URL (network/DNS error): {str(e)}",
                )
            ...
            if _is_blocked_status(status_code):
                return CatalogScrapeResponse(
                    ...
                    blocked_hint="Site returned an anti-bot/forbidden status. Try Woo Store API (if enabled), or use a different site, or implement HTML-paste import.",
                )

            # 2) JSON-LD on initial page
            ...
            # 3) Homelink-style list page parse (works for /catalogue/category/... pages)
            if not created:
                homelink_items = _parse_homelink_list_page(html, base_url=url, limit=min(payload.max_items, 200))
                if homelink_items:
                    # If list page already gives price, ingest directly
            ...
            # 4) Crawl product pages (Shopify/Woo/Homelink)
            if payload.crawl_product_pages and len(created) < payload.max_items:
                ...
                for link in links:
                    ...
                    if _is_blocked_status(r.status_code):
                        continue
                    if r.status_code != 200:
                        continue
                    ...
                    # Shopify product JSON fallback if JSON-LD not present
                    ...
                    # Generic HTML fallback (Homelink/custom sites)
            ...
            # 5) OG fallback (single product pages only — last resort)
    ...
    if not created:
        raise HTTPException(
            ...
            detail="No valid products found to ingest (missing title/price). Try a direct product URL, or ensure crawling is enabled, or the site may be blocking scraping.",
        )
    ...
    # IMPORTANT: return Pydantic models (avoids Pydantic v2 model_type errors)
